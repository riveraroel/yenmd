...
if (!IS_AJAX) {
    ...
    $presc_date = $_POST['prescDate'];
    $blank = "none";
    $is_active = '1';

    // 🔹 Generate deduplication hash
    $presc_hash = hash("sha256", $px_id . "|" . $issued_by . "|" . $presc . "|" . $presc_date);

    // 🔹 Check if hash already exists
    $stmt = $conn->prepare("SELECT presc_id FROM tbl_prescrip WHERE presc_hash = ? LIMIT 1");
    $stmt->bind_param("s", $presc_hash);
    $stmt->execute();
    $stmt->store_result();

    if ($stmt->num_rows > 0) {
        echo "Duplicate prescription detected. The system blocks saving the exact same prescription for the same patient by the same doctor on the same date.";
        $stmt->close();
        $conn->close();
        exit;
    }
    ...
    if ($isRegulated) {
        // Validate patient DOB & Address
        $stmtCheck = $conn->prepare("SELECT px_dob, px_address FROM tbl_px_details WHERE px_id = ?");
        $stmtCheck->bind_param("i", $px_id);
        $stmtCheck->execute();
        $stmtCheck->bind_result($checkDOB, $checkAddress);
        $stmtCheck->fetch();
        $stmtCheck->close();

        if (empty($checkDOB) || empty(trim($checkAddress))) {
            echo "Error: Date of Birth and Address are required for regulated prescriptions.";
            return false;
        }
    }

    // 🔹 Insert with hash
    $s = $conn->prepare("INSERT INTO tbl_prescrip 
        (prescription_for, prescription, issued_by, prescription_ptr, prescription_s2, prescription_date, 
         prescription_folldate, prescription_is_active, presc_filepath, is_regulated, no_refill, presc_hash) 
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");

    $s->bind_param(
        "isissssssiss",
        $px_id, $presc, $issued_by, $ptr, $s2, $presc_date, $flupdate,
        $is_active, $blank, $isRegulated, $noRefill, $presc_hash
    );
    $s->execute();
    $s->store_result();
    ...
    if ($s->affected_rows == 1) {
        $id = $s->insert_id;
            ...
            $s->bind_param("i", $id);
            $s->execute();
            $s->bind_result($email, $fullname, $info_name, $info_licno, $info_address, $info_contact, $px_address, $px_dob, $prescription, $ptr, $s2, $presc_date, $presc_folldate, $isRegulated, $noRefill);
            $s->fetch();
            $s->close();
...
// Place watermark (requires pre-faded transparent PNG)
$watermarkPath = __DIR__ . '/../../../favicon/rod_of_asclepius2.png';

if (file_exists($watermarkPath)) {
    $pageW = $pdf->GetPageWidth();
    $pageH = $pdf->GetPageHeight();

    // Get actual pixel dimensions of the PNG
    list($imgWpx, $imgHpx) = getimagesize($watermarkPath);
    $aspectRatio = $imgHpx / $imgWpx; // height ÷ width

    // Set desired watermark width in mm (adjust as needed)
    $wmWidthMm  = 110;
    $wmHeightMm = $wmWidthMm * $aspectRatio;

    // Center horizontally and vertically
    $xPos = ($pageW - $wmWidthMm) / 2;
    $yPos = ($pageH - $wmHeightMm) / 2 + 23;

    // Insert watermark
    $pdf->Image($watermarkPath, $xPos, $yPos, $wmWidthMm, $wmHeightMm);
}       
...
if (!function_exists('numberToWords')) {
    function numberToWords($number) {
        ...
        // Set image path
        $image_path = '../../../favicon/bannerzxc2.png';
        $page_width = 148; // A5 width in mm
