...
else
{
    ...
    {
        $s = $conn->prepare("INSERT INTO tbl_px_details (px_firstname, px_midname, px_lastname, px_suffix, px_dob, px_gender, px_civilstatus, px_cellnumber, px_emailadd, px_hmo, px_company, issued_by, px_is_active, px_address) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $s->bind_param("ssssssssssssss", $fname, $mname, $lname, $suffix, $dob, $gender, $civstat, $cellnum, $emailadd, $hmo, $company, $issued_by, $is_active, $addr);
        $s->execute();
        $s->store_result();
