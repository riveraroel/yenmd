...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function extractLabValues(text) {
    const refMap = [
      ...
      { name: "SODIUM", useSI: false, aliases: ["SODIUM"] },     // 🔒 removed "Na"
      { name: "POTASSIUM", useSI: false, aliases: ["POTASSIUM"] }, // 🔒 removed "K", "K+"
    ...
    input.addEventListener('change', async function () {
      ...
      reader.onloadend = async function () {
        ...
        try {
          ...
          if (ocrOutputEl) ocrOutputEl.value = text;

          document.getElementById('ocrRawText').textContent = text;

          const extracted = extractLabValues(text);
          document.getElementById('labresult').value = extracted;

        } catch (err) {
          console.error('Vision API error:', err);
          document.getElementById('ocrRawText').textContent = '❌ Error using Vision API.';
        }
      };
