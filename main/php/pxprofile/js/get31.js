...
function parse(text) {
  ...
  text.split('\n').forEach((line, index) => {
    ...
    console.log(`\n[Line ${index + 1}] Original: "${line}"`);

    // ✅ If no digits, it's not a lab value — preserve as-is
    if (!/\d/.test(line)) {
      rawMap[line] = null;
      console.log(`  ⏭️ No numbers found — preserved as-is`);
      return;
    }
    ...
    // Try common formats: "TEST: VALUE", "TEST - VALUE"
    let match = line.match(/^(.+?)\s*[:\-]\s*(.+)$/);
    if (match) {
      key = match[1].trim();
      val = match[2].trim();
    } else {
      // If no punctuation, try: "TESTNAME VALUE"
      ...
      if (!isNaN(num)) {
        ...
        rawMap[line] = null;
        console.log(`  ⚠️ Cannot extract key/value — preserved as-is`);
        return;
      }
    }
    ...
    if (refRanges.hasOwnProperty(cleanedKey)) {
      finalKey = cleanedKey;
      console.log(`  ✅ Recognized test name: "${finalKey}"`);
    } else if (aliases.hasOwnProperty(cleanedKey)) {
      finalKey = aliases[cleanedKey];
      console.log(`  🔁 Alias applied: "${cleanedKey}" → "${finalKey}"`);
    } else {
      finalKey = cleanedKey;
      console.log(`  ➕ Custom key (no alias): "${finalKey}"`);
    }
    ...
    if (!isNaN(numeric)) {
      numMap[finalKey] = numeric;
      console.log(`  🔢 Parsed numeric: ${numeric}`);
    } else {
      console.log(`  ℹ️ Non-numeric value: "${val}"`);
    }
  });

  console.log('\n✅ Final parsed keys:', rawMap);
...
return Object.entries(current).map(([key, rawVal]) => {
  if (rawVal === null) return key; // preserve untouched lines like "UTZ: NORMAL"
  ...
  if (!range || val === undefined) {
    ...
    const tooltip = isMinOnly
      ? `Normal range: ${min} and above`
      : `Normal range: ${min}–${max}`;

let arrow = '';
if (prevVal !== undefined && val !== prevVal) {
  const improving = wasNormal === false && nowNormal === true;
  const worsening = wasNormal === true && nowNormal === false;

  if (val > prevVal) {
    arrow = `<i class="fas fa-arrow-up ${worsening ? 'text-danger' : improving ? 'text-success' : ''}" title="${worsening ? 'Worsening' : improving ? 'Improving' : 'Increased'}" data-bs-toggle="tooltip"></i>`;
  } else if (val < prevVal) {
    arrow = `<i class="fas fa-arrow-down ${worsening ? 'text-danger' : improving ? 'text-success' : ''}" title="${worsening ? 'Worsening' : improving ? 'Improving' : 'Decreased'}" data-bs-toggle="tooltip"></i>`;
  }

  if (arrow) arrow = `&nbsp;${arrow}`; // ✅ Add spacing before the arrow
}

    const valueHTML = nowNormal
      ? val
      : `<span style="color:red;" data-bs-toggle="tooltip" title="${tooltip}">${val}</span>`;
...
function labResultRenderer(data, type, row) {
  if (!data) return '';
  const escapedData = escapeHtml(data); // ONLY for attribute
  return `<div class="labresult-highlight" data-raw="${escapedData}"></div>`;
}
...
// 🧠 Special case for URIC ACID: always pick 2 rows above (SI unit)
...
if (value) {
  ...
  continue; // prevent fallback logic from overriding
      ...
      // FT3 / FT4 / TSH → value is one line below
      ...
      // VITAMIN D → value is 2–3 lines above, skip non-numeric lines
      ...
      // Default: Look 1–2 lines above for value
      else {
        for (let j = 1; j <= 2; j++) {
