...
function populatePatientInfo() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      // 🔵 Titles and labels
      $('#viewPx_Name').html(px.fullname);
      $('#addDiagnosisLabel').html("Add Diagnosis - " + px.fullname);
      $('#viewDiagnosisLabel').html("View / Edit Diagnosis - " + px.fullname);
      $('#bookAppointmentLabel').html("Book Appointment - " + px.fullname);
      $('#viewAppointmentLabel').html("View / Edit Appointment - " + px.fullname);
      $('#addPrescriptionLabel').html("Add Prescription - " + px.fullname);
      $('#viewPrescriptionLabel').html("View / Edit Prescription - " + px.fullname);
      $('#requestLabTestLabel').html("Add Labs - " + px.fullname);
      $('#viewEditLabTestLabel').html("View / Edit Labs - " + px.fullname);

      // 🔵 Shared input fields
      $('#prescAddress').val(px.px_address);
      $('#view_prescAddress').val(px.px_address);
      $('#mobile_fullname').html(px.fullname);
      ...
      if (rawDob) {
        ...
        patientDobDate = dobDate; // ← store globally
        patientIsPWD = px.px_is_pwd == 1;
        ...
        ageShort = getShortAgeString(dobDate);  // e.g., 69y 10m 14d
        ageFull = `(${ageShort})`;

        const ageYears = new Date().getFullYear() - dobDate.getFullYear();
        if (ageYears >= 60 || px.px_is_pwd == 1) {
          seniorIcon = `<span class="ms-1" title="${ageYears >= 60 && px.px_is_pwd == 1 ? 'Senior + PWD' : (ageYears >= 60 ? 'Senior' : 'PWD')}">♿</span>`;
        }

      }

      // 🔵 Set subtitle (desktop only)
      const subtitleParts = [];

      if (px.px_id) subtitleParts.push(`Patient ID: ${px.px_id}`);

      if (px.px_gender && px.px_gender !== "None") {
        const genderIcon = px.px_gender === "Male"
          ? `<i class="bi bi-gender-male text-primary" title="Male"></i>`
          : `<i class="bi bi-gender-female" style="color: hotpink;" title="Female"></i>`;
        subtitleParts.push(`${px.px_gender} ${genderIcon}`);
      }

      if (formattedDob) subtitleParts.push(`${formattedDob} ${ageFull} ${seniorIcon}`);

      $('#viewPx_Subtitle').html(subtitleParts.join(' · '));

      // 🔵 Mobile card values
      if (px.px_gender && px.px_gender !== "None") {
        let genderDisplay = px.px_gender;
        if (px.px_gender === "Male") {
          genderDisplay += ` <i class="bi bi-gender-male text-primary" title="Male"></i>`;
        } else if (px.px_gender === "Female") {
          genderDisplay += ` <i class="bi bi-gender-female" title="Female" style="color: hotpink;"></i>`;
        }
        $('#mobile_gender').html(genderDisplay);
      } else {
        $('#mobile_gender').html('');
      }

      $('#mobile_dob').text(formattedDob);
      $('#mobile_age').html(ageShort + seniorIcon);
      $('#mobile_civstat').text(px.px_civilstatus === "None" ? '' : px.px_civilstatus);
      $('#mobile_cellnum').text(px.px_cellnumber);
      $('#mobile_emailadd').text(px.px_emailadd);
      $('#mobile_addr').text(px.px_address);
      $('#mobile_hmo').text(px.px_hmo === "None" ? '' : px.px_hmo);
      $('#mobile_company').text(px.px_company);

      // 🔵 Desktop form fields
      setTimeout(() => {
        $('#viewpx_fname').val(px.px_firstname);
        $('#viewpx_mname').val(px.px_midname);
        $('#viewpx_lname').val(px.px_lastname);
        $('#viewpx_suffix').val(px.px_suffix);
        $('#viewpx_dob').val(px.px_dob);
        $('#viewpx_gender').val(px.px_gender);
        $('#viewpx_civstat').val(px.px_civilstatus);
        $('#viewpx_cellnum').val(px.px_cellnumber);
        $('#viewpx_emailadd').val(px.px_emailadd);
        $('#viewpx_addr').val(px.px_address);
        $('#viewpx_is_pwd').prop('checked', px.px_is_pwd == 1);
        $('#viewpx_hmo').val(px.px_hmo);
        $('#viewpx_company').val(px.px_company);
      }, 50);
    },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (patientDobDate instanceof Date) {
    const today = new Date();
    age = today.getFullYear() - patientDobDate.getFullYear();
  ...
  if (isSenior || patientIsPWD) {
    ...
    // 🔵 Dynamic notice
    let reason = [];
    if (isSenior) reason.push("Senior");
    if (patientIsPWD) reason.push("PWD");

    ptrNotice.textContent = `PTR auto-applied for ${reason.join(" + ")}`;
