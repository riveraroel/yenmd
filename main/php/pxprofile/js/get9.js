  ...
  $.ajax({
    ...
    success: function(data) {
      if ($.isArray(data)) {
        ...
        $('#addDiagnosisLabel').html("Add Diagnosis - " + data[0].fullname)
        ...
        const rawDob = data[0].px_dob;

        if (rawDob) {
          const [year, month, day] = rawDob.split('-');
          const dobDate = new Date(year, month - 1, day); // JS months are 0-based
          ...
          $('#mobile_age').text(ageShort);
        } else {
          $('#mobile_dob').text("");
          $('#mobile_age').text("");
        }

        $('#mobile_gender').text(data[0].px_gender === "None" ? '' : data[0].px_gender);
        $('#mobile_civstat').text(data[0].px_civilstatus === "None" ? '' : data[0].px_civilstatus);
        $('#mobile_cellnum').text(data[0].px_cellnumber);
        $('#mobile_emailadd').text(data[0].px_emailadd);
        $('#mobile_addr').text(data[0].px_address);
        $('#mobile_hmo').text(data[0].px_hmo === "None" ? '' : data[0].px_hmo);
