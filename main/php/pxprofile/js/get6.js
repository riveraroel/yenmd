$(document).ready(function(){
  ...
  columnDefs: [
    ...
    },
    {
      ...
      width: '10%',
      className: 'text-center',
      defaultContent: "<button type='button' class='btn btn-outline-primary' id='view_preshistory'>View</button>"
    }
  ]
});
...
var tb_medhistory = $('#tb_medhistory').DataTable({
  ...
  columnDefs: [
    ...
    },
    {
      ...
      className: 'text-center',
      data: null,
      defaultContent: "<button type='button' class='btn btn-outline-primary' id='view_medhistory'>View</button>"
    }
  ]
});
    ...
    autoWidth: false,
        ...
        columnDefs: [
          {
            data: null,
            defaultContent: "<button type = 'button' class = 'btn btn-outline-primary' id = 'view_apt_history'>View</button>",
            targets: -1
          }
        ]
     });
  ...
  columnDefs: [
    {
      targets: -1,
      data: null,
      defaultContent: "<button type='button' class='btn btn-outline-primary' id='view_labtest_history'>View</button>",
      width: '10%'
    },
    {
      targets: 0,
...
//Appointment History View button//
  ...
  let apt_id = $(this).closest('tr').attr('id');
  ...
  $(document).on('click', '#view_preshistory', function () {
    presc_id = $(this).closest('tr').attr('id');
...
// // Prescription history View button
// $(document).on('click', '#view_preshistory', function () {
//   presc_id = $(this).closest('tr').attr('id');
//   $('#viewPrescriptionForm')[0].reset();
//   $('#viewPrescription').modal('show');
...
//         // PTR field always readonly
//         $('#view_prescPTR').val(presc.ptr || '').prop('readonly', true);

//         if (isRegulated) {
//           // Hide PTR toggle
//           $('#view_includePtrToggle').hide();

//           // Show PTR and S2 wrappers
//           $('#view_ptrWrapper').show();
//           $('#view_s2Wrapper').removeClass('d-none');

//           // S2 input readonly and filled
//           $('#view_prescS2').val(presc.s2 || '').prop('readonly', true);

//           // All other fields editable already set above

//         } else {
//           if (originalPtrValue !== '') {
//           // Non-regulated: show PTR toggle
//           $('#view_includePtrToggle').show();

//           // Show/hide PTR input based on toggle and ptr value
//           $('#view_includePtr').prop('checked', true);
//           $('#view_prescPTR').show().val(originalPtrValue);
//         } else {
//           $('#view_includePtr').prop('checked', false);
//           $('#view_prescPTR').hide().val('');
//         }

//           // Hide S2 wrapper
//           $('#view_s2Wrapper').addClass('d-none');
//           $('#view_prescS2').val('');
//         }
//       }
//     },
...
// // Update the toggle logic to restore the value if shown
// $('#view_includePtr').on('change', function () {
//   if ($(this).is(':checked')) {
//     $('#view_prescPTR').show();
//     if (!$('#view_prescPTR').val()) {
//       $('#view_prescPTR').val(originalPtrValue);
//     }
//   } else {
//     $('#view_prescPTR').hide().val('');
//   }
...
$(document).on('click', '#view_labtest_history', function () {
  labtest_id = $(this).closest('tr').attr('id');
...
$(document).on('click', '#view_medhistory', function () {
  medhis_id = $(this).closest('tr').attr('id');
...
$('#delete_labtest').click(function(){
  ...
  var labtest_request_date = $('#edit_labtest_request_date').val();
  var labtest_clinical_impression = $('#edit_labtest_clinical_impression').val();
  var labtest_remarks = $('#edit_labtest_remarks').val();
  var labtest_folldate = $('#edit_labtest_folldate').val();
  var labtest_others = $('#edit_otherTests').val();
