...
$('#prescription').on('input', function () {
  ...
  const isRegulatedChecked = $('#isRegulated').is(':checked');

  const matchedDrug = regulatedDrugs.find(drug => text.includes(drug));

  if (matchedDrug && !isRegulatedChecked && !regulatedAlertShown) {
    regulatedAlertShown = true; // prevent repeat alerts
    Swal.fire({
      icon: 'warning',
      title: 'Regulated Drug Detected',
      text: `The medicine "${matchedDrug}" appears to be a regulated drug. Please ensure the regulated toggle is turned ON.`,
    }).then(() => {
      regulatedAlertShown = false; // reset after user closes the alert
    });
  }
});
