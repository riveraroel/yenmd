...
if (!value) {
  ...
  // Try joining the first 2 parts to catch things like "VIT D"
  const twoPartName = (parts[0] + ' ' + (parts[1] || '')).toUpperCase();

  if (aliases[twoPartName]) {
    name = twoPartName;
    value = parts[2];
  } else {
    name = parts[0];
...
function highlightOutliers(labText, px_gender = 'Male') {
  ...
  return labText.split('\n').map(line => {
    ...
    if (line.includes(':')) {
      ...
      const parts = line.trim().split(/\s+/);
      const twoPartKey = (parts[0] + ' ' + (parts[1] || '')).toUpperCase();

      if (aliases[twoPartKey]) {
        keyRaw = twoPartKey;
        valRaw = parts[2] || '';
      } else {
        keyRaw = parts[0];
    ...
    const [min, max] = range;
    const isMinOnly = normalizedKey === 'HDL CHOLE' || normalizedKey === 'VITAMIN D';
    const isOutlier = isMinOnly
      ? val < min
      : val < min || val > max;
