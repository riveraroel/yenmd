...
document.addEventListener('DOMContentLoaded', () => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const ref of refMap) {
      ...
      const rowParts = currentLine.split(/\s+/);
      if (rowParts.length >= 3) {
        if (ref.name === "URIC ACID") {
          const siMatch = rowParts[2]?.match(/[\d.]+/);
          if (siMatch) value = siMatch[0];
        } else {
          const convMatch = rowParts[1]?.match(/[\d.]+/);
          if (convMatch) value = convMatch[0];
        }
      }

      // If not found via columns, use original rules
      if (!value) {
        ...
        reader.onloadend = async function () {
          ...
          try {
            ...
            if (uploadResult.success) {
              ...
              refreshUploadedImages(); // ✅ Refresh thumbnails
            } else {
...
const res = await fetch(`${basePath}/render_uploads.php?px_id=${pxId}`);
    ...
    console.log('📦 Received HTML from render_uploads:', html); // <-- Add this
    container.innerHTML = html;
    rebindImageEvents(); // This rebinds click/delete/tag logic
  } catch (err) {
