...
function parse(text) {
  ...
  text.split('\n').forEach(line => {
    ...
    const cleanedKey = rawKey.toUpperCase().replace(/\s+/g, ' ');
    const key = aliases[cleanedKey] || cleanedKey;
  ...
  const current = parse(labText);
  const previous = parse(prevLabText);

  return Object.entries(current).map(([key, val]) => {
    const range = refRanges[key];
    if (!range) return `${key}: ${val}`;
    ...
    const wasNormal = (() => {
      ...
      return isMinOnly ? prevVal >= min : prevVal >= min && prevVal <= max;
    })();
    ...
    const isOut = !nowNormal;
    let arrow = '';

    const prevVal = previous[key];
    if (prevVal !== undefined && (isOut || wasNormal === false || wasNormal === true)) {
      const improving = !nowNormal && wasNormal === false && val < prevVal;
      const worsening = nowNormal === false && wasNormal === true;
      const normalized = wasNormal === false && nowNormal === true;

      if (val > prevVal) {
        if (worsening) {
          arrow = ` <i class="fas fa-arrow-up text-danger" data-bs-toggle="tooltip" title="Worsening"></i>`;
        } else if (normalized) {
          arrow = ` <i class="fas fa-arrow-up text-success" data-bs-toggle="tooltip" title="Improving"></i>`;
        }
      } else if (val < prevVal) {
        if (worsening) {
          arrow = ` <i class="fas fa-arrow-down text-danger" data-bs-toggle="tooltip" title="Worsening"></i>`;
        } else if (normalized || improving) {
          arrow = ` <i class="fas fa-arrow-down text-success" data-bs-toggle="tooltip" title="Improving"></i>`;
        }
      }
    }

    const valueHTML = isOut
