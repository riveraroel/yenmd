...
$(document).on('click', '#view_preshistory', function () {
  ...
  $.ajax({
    ...
    dataType: "json",
    data: {
      presc_id: btoa(presc_id)
    },
    success: function (data) {
      if ($.isArray(data)) {
        $('#view_prescDate').val(data[0].presc_date);
        $('#view_prescription').val(data[0].presc);
        $('#view_prescFollDate').val(data[0].presc_folldate);
        $('#view_prescPTR').val(data[0].ptr);
        $('#view_prescS2').val(data[0].s2);
        $('#dl_tag').attr('href', data[0].fl);
      }
    },
    error: function (xhr, ajaxoption, thrownerror) {
      console.log("Get Prescription History Error: " + xhr.status + " " + thrownerror);
    }
  });
});
...
$(document).on('click', '#view_labtest_history', function () {
  const labtest_id = $(this).closest('tr').attr('id');
  ...
  $.ajax({
    ...
    error: function (xhr, ajaxoption, thrownerror) {
     ...
     $('#update_prescription').click(function(){
      var updpresc = $('#addPrescriptionForm2').serializeArray();
      var prid = 'presc_id='+presc_id;
      var serializedupdPrescriptionForm = updpresc.map(function(field) {
        return field.name + '=' + encodeURIComponent(field.value);
      }).join('&');
      serializedupdPrescriptionForm += '&' + prid;
      $.ajax({
        type: 'POST',
        url: 'update_prescription.php',
        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
        dataType: 'text',
        data: serializedupdPrescriptionForm,
        cache: false,
        success: function(data)
        {
          alert(data);
