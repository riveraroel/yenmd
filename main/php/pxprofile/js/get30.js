...
function parse(text) {
  ...
  text.split('\n').forEach(line => {
    ...
    // Match: longest possible test name ending before a number
    const match = line.match(/^(.+?)\s*[:\-]?\s*([0-9.,]+)$/);
    if (!match) {
      rawMap[line] = ''; // e.g., "UTZ NORMAL", "2D ECHO - NORMAL"
      return;
    }

    let [, rawKey, rawVal] = match;
    const cleanedKey = rawKey.toUpperCase().replace(/\s+/g, ' ').trim();

    // Avoid aliasing if it's already a known test
    const key = refRanges.hasOwnProperty(cleanedKey)
      ? cleanedKey
      : (aliases[cleanedKey] || cleanedKey);

    const val = rawVal.trim();
    rawMap[key] = val;

    const num = parseFloat(val.replace(',', ''));
    if (!isNaN(num)) numMap[key] = num;
  });
