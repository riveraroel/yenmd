...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const pdfModal = new bootstrap.Modal(pdfModalEl, { backdrop: true, keyboard: true });

  const imageModalEl = document.getElementById('imageModal');
  const imageModal = new bootstrap.Modal(imageModalEl, { backdrop: true, keyboard: true });
  ...
  function waitForPDFJS(callback) {
    ...
    else setTimeout(() => waitForPDFJS(callback), 100);
  }
...
async function renderAllPages(url) {
  try {
    ...
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
      ...
      // Page wrapper (adds separation like sheets)
      ...
      pageWrapper.style.boxShadow = "0 2px 6px rgba(0,0,0,0.15)";
      ...
      canvas.style.margin = "0.5rem auto 0.5rem auto";
  ...
  window.openImageModal = function(imgUrl) {
    ...
    document.getElementById("imageDownload").href = imgUrl;
    imageModal.show();
  };
  ...
  // ==========================
  document.getElementById('pdfCloseBtn')?.addEventListener('click', () => pdfModal.hide());
  document.getElementById('imageCloseBtn')?.addEventListener('click', () => imageModal.hide());
  ...
  // 🏷 Save lab tag (static binding - attach only once)
const saveBtn = document.getElementById('saveLabTag');
if (saveBtn) {
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (data.success) {
        setTimeout(() => refreshUploadedImages(), 300);
      } else {
  ...
  function extractLabValues(text) {
    ...
    if (
      (
        textUpper.includes("ECHOCARDIOGRAM") ||
        textUpper.includes("ECHOCARDIOGRAPHY") ||
        textUpper.includes("DOPPLER") ||
        textUpper.includes("COLOR FLOW")
      ) && !alreadyFound.has("2D ECHO")
    ) {
      results.push("2D ECHO: CHECK UPLOADED");
      alreadyFound.add("2D ECHO");
    }
...
// (Prescription handling + lab result highlighting remain unchanged...)
...
async function refreshUploadedImages() {
  ...
  try {
    ...
    rebindUploadedEvents();
  } catch (err) {
    console.error('Error refreshing uploads:', err);
  }
}

// Rebind all events after render
function rebindUploadedEvents() {
  // 🖼 Image modal
  ...
  document.querySelectorAll('.open-pdf-modal').forEach(pdf => {
    pdf.addEventListener('click', () => openPdfModal(pdf.dataset.pdf));
  });
  ...
  // 🏷 Tag lab modal buttons
  document.querySelectorAll('.tag-lab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const id = btn.dataset.uploadId;
      const currentLabType = btn.dataset.labType || '';
      const select = document.getElementById('tagLabSelect');
      document.getElementById('tagUploadId').value = id;
      if (select) select.value = currentLabType;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tooltipTriggerList.map(function (tooltipTriggerEl) {
    return new bootstrap.Tooltip(tooltipTriggerEl, {
      delay: { "show": 0, "hide": 100 } // instant show, quick hide
    });
  });
});
