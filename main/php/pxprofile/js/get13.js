  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      if (rawDob) {
        ...
        ageYears = new Date().getFullYear() - dobDate.getFullYear();
      ...
      let iconLabel = '';
      if (ageYears !== null && ageYears >= 60 && px.px_is_pwd == 1) iconLabel = 'Senior + PWD';
      else if (ageYears !== null && ageYears >= 60) iconLabel = 'Senior';
      else if (px.px_is_pwd == 1) iconLabel = 'PWD';

      const statusIcon = iconLabel ? `<span class="ms-1" title="${iconLabel}">♿</span>` : '';
      ...
      if (formattedDob) {
        subtitleParts.push(`${formattedDob} ${ageFull} ${statusIcon}`);
      } else if (statusIcon) {
        subtitleParts.push(statusIcon); // show ♿ icon even if no DOB
      }
      ...
      $('#mobile_age').html((ageShort || '') + (statusIcon || ''));
