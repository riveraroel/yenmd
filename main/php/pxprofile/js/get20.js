...
function sanitizeLabInput(text) {
  const refRanges = {
    FBS: [60, 99],
    BUN: [4.76, 23.25],
    CREATININE: [0.5, 1.20],
  ...
  return text
    ...
    .map(line => {
      ...
      if (!name || !value) return ''; // Skip blank or invalid
      ...
      const [min, max] = range;
      const isOutlier = numeric < min || numeric > max;

      return isOutlier
        ? `${normalized}: <span style="color:red;" data-bs-toggle="tooltip" title="Normal range: ${min}–${max}">${numeric}</span>`
        : `${normalized}: ${numeric}`;
    })
    .filter(Boolean)
    .join('<br>');
...
function highlightOutliers(labText, px_gender = 'Male') {
  px_gender = (px_gender || 'Male').toLowerCase(); // ✅ normalize
  console.log('🧪 highlightOutliers gender param:', px_gender);
  // Define normal ranges by default or by sex where applicable
  const refRanges = {
    FBS: [60, 99],
    BUN: [4.76, 23.25],
    // Creatinine differs by sex:
    CREATININE: px_gender.toLowerCase() === 'female' ? [0.5, 1.0] : [0.7, 1.2],
    'URIC ACID': [149, 405],
    CHOLESTEROL: [0, 200],
    TRIGLYCERIDES: [35, 160],
    // HDL: values > 60 are good, so only flag if < 40
    ...
    // 🔍 Debug range being used for Creatinine
  if (labText.includes('CREATININE')) {
    console.log('👀 Creatinine range:', refRanges.CREATININE, 'Gender:', px_gender);
  }
  ...
  return labText.split('\n').map(line => {
    ...
    if (!range || isNaN(val)) return line;

    const [min, max] = range;
    let isOutlier;

    if (key === 'HDL CHOLE') {
      // HDL: outlier only if below 40
      isOutlier = val < min;
    } else {
      isOutlier = val < min || val > max;
    }

    const tooltip = key === 'HDL CHOLE'
      ? `Normal range: 40 and above`
      : `Normal range: ${min} – ${max}`;

    return isOutlier
      ? `${keyRaw}: <span style="color:red;" data-bs-toggle="tooltip" title="${tooltip}">${valRaw}</span>`
      : `${keyRaw}: ${valRaw}`;
  }).join('<br>');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('submit_diagnosis')?.addEventListener('click', () => {
    ...
    const preview = document.getElementById('viewLabResult'); // or wherever you show it
    const cleaned = sanitizeLabInput(raw);

    if (preview) {
      preview.innerHTML = cleaned;

      // Re-initialize tooltips for outliers
      const tooltips = [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'));
      tooltips.forEach(el => new bootstrap.Tooltip(el));
    }

    // OPTIONAL: store clean version
    // document.getElementById('labresult').value = raw;
  });
