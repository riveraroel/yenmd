...
function highlightOutliers(labText) {
  const refRanges = {
    FBS: [60, 99],
    BUN: [4.76, 23.25],
    CREATININE: [0.5, 1.0],
  ...
  return labText.split('\n').map(line => {
    ...
    const [min, max] = range;
    const isOutlier = val < min || val > max;
    const tooltip = `Normal range: ${min} – ${max}`;
...
function updateLabResultHighlights() {
  document.querySelectorAll('.labresult-highlight').forEach(el => {
    const raw = el.getAttribute('data-raw');
    if (raw) {
      el.innerHTML = highlightOutliers(raw);
    }
  });

  // Initialize Bootstrap tooltips on any new elements with data-bs-toggle="tooltip"
...
document.getElementById("submit_appointment").addEventListener("click", function () {
    const px_id = document.getElementById("selected_px_id").value;
    ...
    if (!px_id || !date || !start || !end || !reason) {
        Swal.fire({
            icon: 'warning',
    ...
    formData.append("px_id", px_id);
    formData.append("appointmentDate", date);
    formData.append("appointmentStart", start);
    formData.append("appointmentEnd", end);
    formData.append("reason", reason);

    Swal.fire({
        title: 'Please wait...',
        html: 'Booking appointment and sending confirmation email.',
    ...
    .then(msg => {
        ...
        // Close modal
        const modal = bootstrap.Modal.getInstance(document.getElementById("bookAppointment"));
        modal.hide();

        // Optional: refresh appointment history or calendar
    })
    .catch(error => {
