...
else
{
    ...
    if ($s->affected_rows == 1)
    {
        ...
        $s = $conn->prepare("SELECT px.px_emailadd AS email, 
                            ...
                            mh.medhis_attachment AS pdf_att, 
        ...
        $s->execute();
        $s->bind_result($email, $firstname, $lastname, $suffix, $company, $labresult, $diag, $medication, $recom, $pdfatt, $diag_date, $issue_date);
        $s->fetch();
        $s->close();
        ...
        $line_x_start = $x_position;
        $line_x_end = $x_position + $cell_width;
        $line_y_position = $y_position + 12;       
        $pdf->Line($line_x_start, $line_y_position, $line_x_end, $line_y_position);      
