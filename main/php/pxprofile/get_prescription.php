...
$res = $s->get_result();
    ...
    while ($row = $res->fetch_assoc()) {
        // Calculate age from px_dob
        if (!empty($row['px_dob']) && $row['px_dob'] !== '0000-00-00') {
            $dob = new DateTime($row['px_dob']);
            $today = new DateTime();
            $interval = $dob->diff($today);
            $row['px_age'] = $interval->y . 'y ' . $interval->m . 'm';
        } else {
            $row['px_age'] = '';
        }
