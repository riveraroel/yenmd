...
$issued_by = $_SESSION['USER_ID'] ?? 0; // fallback to 0 if not set
...
// Validate and sanitize date/time inputs
...
// Sanitize reason - strip tags and encode special chars
$reason = htmlentities(trim(urldecode($reasonRaw)), ENT_QUOTES | ENT_HTML5, 'UTF-8');
...
if ($durationHours <= 0) {
    ...
    exit("Invalid time. Please try again.");
}

$is_attended = '2'; // pending
$is_active = '1';
...
if ($checkStmt->num_rows > 0) {
    ...
    exit("This appointment slot is already taken. Please choose another time.");
}
$checkStmt->close();
...
if ($insertStmt->affected_rows !== 1) {
    ...
    exit("Failed to add appointment. Please try again.");
}
...
if (empty($email)) {
    ...
    exit("Appointment has been added, but no email address found.");
}
...
if (empty($host) || empty($username) || empty($password)) {
    http_response_code(200);
    exit("Appointment added, but SMTP settings are incomplete.");
}
...
HTML;
    ...
    // Optional: log error to file or error logging system here
    http_response_code(500);
    echo "Appointment saved, but email failed: {$mail->ErrorInfo}";
}
