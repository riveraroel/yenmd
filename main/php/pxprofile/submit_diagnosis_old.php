...
else
{
    ...
    $is_active = '1';
    $s = $conn->prepare("INSERT INTO tbl_medical_history (px_id, issued_by, medhis_labresult, medhis_diagnosis, medhis_medication, medhis_recommendation, medhis_diagnosis_date, medhis_issuance_date, medhis_attachment, medhis_is_active) VALUES (?,?,?,?,?,?,?,?,?,?)");
    $s->bind_param("iissssssss", $px_id, $issued_by, $labresult, $diag, $medication, $recom, $diagdate, $issue_date, $blank, $is_active);
    $s->execute();
    $s->store_result();
    if ($s->affected_rows == 1)
    {
        $medhis_id = $s->insert_id;
        $s = $conn->prepare("SELECT px.px_emailadd AS email, CONCAT(px.px_firstname, ' ', px.px_midname, ' ', px.px_lastname, ' ', px.px_suffix) AS fullname, px.px_company AS company, mh.medhis_labresult AS labresult, mh.medhis_diagnosis AS diag, mh.medhis_medication AS medication, mh.medhis_recommendation AS recom, mh.medhis_attachment AS pdf_att, mh.medhis_diagnosis_date AS diag_date, mh.medhis_issuance_date AS issdate FROM tbl_medical_history AS mh INNER JOIN tbl_px_details AS px ON mh.px_id = px.px_id WHERE mh.medhis_id = ?");
        $s->bind_param("i", $medhis_id);
        $s->execute();
        $s->bind_result($email, $fullname, $company, $labresult, $diag, $medication, $recom, $pdfatt, $diag_date, $issued_date);
        $s->fetch();
        $s->close();
