...
// }
    ...
    $s = $conn->prepare("UPDATE tbl_prescrip  SET issued_by = ?, prescription = ?, prescription_ptr = ?, prescription_s2 = ?, prescription_date = ?, prescription_folldate = ?, is_regulated = ? WHERE presc_id = ?");
    $s->bind_param("isssssii", $issued_by, $presc, $ptr, $s2, $presc_date, $flupdate, $isRegulated, $presc_id);
    $s->execute();
