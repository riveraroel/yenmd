...
$labtest_request_date = isset($_POST['labtest_request_date']) ? $_POST['labtest_request_date'] : '';
...
$insert_stmt = $conn->prepare("INSERT INTO tbl_lab_requests 
    (px_id, labtest_request_date, labtest_selected, labtest_others, labtest_clinical_impression, labtest_remarks, issued_by, pdf_filename, labtest_cre_datetime, labtest_last_update, labtest_folldate) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
if (!$insert_stmt) {
    echo "Insert prepare failed: (" . $conn->errno . ") " . $conn->error;
    exit;
}
$insert_stmt->bind_param("isssssissss", $px_id, $labtest_request_date, $labtest_selected, $labtest_others, $labtest_clinical_impression, $labtest_remarks, $issued_by, $pdf_filename, $now, $now, $labtest_folldate);
...
$image_width_mm = 91.1458;
$image_height_mm = 26.4583;
$max_width_mm = 120;
...
$x_centered = (148 - $image_width_mm) / 2;
$y_position = 5;
...
$conn->close();
echo "Lab test successfully added and PDF generated.";
