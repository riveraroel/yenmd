...
if (!IS_AJAX) {
    ...
    $presc = trim($_POST['view_prescription']); 
...
if ($isRegulated) {
    $sqlCheck = "
        SELECT px.px_dob, px.px_address
    ...
    $stmtCheck->bind_param("i", $presc_id);
    $stmtCheck->execute();
    $row = $stmtCheck->get_result()->fetch_assoc();
    $stmtCheck->close();

    $checkDOB = $row['px_dob'] ?? null;
    $checkAddress = isset($row['px_address']) ? trim($row['px_address']) : null;

    if (empty($checkDOB) || empty($checkAddress)) {
