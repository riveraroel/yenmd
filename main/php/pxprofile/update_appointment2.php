...
else
{
    ...
    if ($seconds_diff > 0)
    {
        $s = $conn->prepare("UPDATE tbl_appointment SET apt_date = ?, apt_start = ?, apt_end = ?, apt_reason = ? WHERE apt_id = ?");
        $s->bind_param("ssssi", $apt_date, $apt_start, $apt_end, $apt_reason, $apt_id);
        $s->execute();
        $s->store_result();
        if ($s->affected_rows == 1)
        {
            ...
            try{
                ...
                $mail->Body .= '<hr>';
                $mail->Body .= '<center><h1>This appointment has been updated</h1></center><br>';
