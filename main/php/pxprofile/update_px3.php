...
else
{
    ...
    $s = $conn->prepare("UPDATE tbl_px_details SET px_firstname = ?, px_midname = ?, px_lastname = ?, px_suffix = ?, px_dob = ?, px_gender = ?, px_civilstatus = ?, px_cellnumber = ?, px_emailadd = ?, px_hmo = ?, px_company = ?, px_address = ?, px_is_pwd = ? WHERE px_id = ?");
    $s->bind_param("ssssssssssssii", $fname, $mname, $lname, $suffix, $dob, $gender, $civstat, $cellnum, $emailadd, $hmo, $company, $addr, $is_pwd, $px_id);
    $s->execute();
    $s->store_result();
