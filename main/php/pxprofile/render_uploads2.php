...
if (empty($uploadImages)) {
  echo "<div class='text-muted'>No images uploaded yet.</div>";
  exit;
}
...
>
  🏷 Change Lab Type
          ...
          <li>
            <button class="dropdown-item text-danger delete-image-btn" data-upload-id="<?= $img['id'] ?>">🗑 Delete</button>
          </li>
        </ul>
      </div>
    <?php endif; ?>
  ...
  <?php if ($isUntagged): ?>
    <button class="btn btn-sm btn-warning w-100 mt-2 tag-lab-btn" data-upload-id="<?= $img['id'] ?>">
      ⚠️ Tag Lab Type
    </button>
  <?php endif; ?>
</div>
...
// ✅ Render all in one flex row, grouped together by lab type
