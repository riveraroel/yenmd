...
if (!IS_AJAX) {
    ...
    if ($isRegulated) {
        // Fetch patient DOB and Address for validation
    ...
    $s = $conn->prepare("INSERT INTO tbl_prescrip (prescription_for, prescription, issued_by, prescription_ptr, prescription_s2, prescription_date, prescription_folldate, prescription_is_active, presc_filepath, is_regulated, no_refill) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
    $s->bind_param("isissssssii", $px_id, $presc, $issued_by, $ptr, $s2, $presc_date, $flupdate, $is_active, $blank, $isRegulated, $noRefill);
    $s->execute();
    $s->store_result();
