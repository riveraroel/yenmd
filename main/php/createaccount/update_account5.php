...
else
{
    ...
    if ($s->num_rows == 0)
    {
        ...
        $z->execute();
        $x = $conn->prepare("UPDATE tbl_information SET information_name = ?, information_licno = ?, information_address = ?, information_contact = ?, information_emailadd = ?, role_id = ? WHERE information_id = ?");
        $x->bind_param("ssssssi", $name, $licno, $address, $contact, $email, $role, $acc_id);
        $x->execute();
