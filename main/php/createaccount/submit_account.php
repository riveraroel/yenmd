...
else
{
    ...
    $ptr = urldecode($_POST['addacc_ptr']);
    $s2 = urldecode($_POST['addacc_s2']);
    ...
    if ($s->num_rows == 0)
    {
        $s = $conn->prepare("INSERT INTO tbl_information (information_emailadd, information_contact, information_name, information_licno, information_ptr, information_s2, information_address, information_is_active, role_id) VALUES (?,?,?,?,?,?,?,?,?)");
        $s->bind_param("ssssssssi", $emailadd, $contact, $name, $licno, $ptr, $s2, $address, $is_active, $role);
        $s->execute();
        $s->store_result();
        $info_id = $s->insert_id;
