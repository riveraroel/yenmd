...
else
{
    ...
    $s = $conn->prepare("SELECT info.information_contact AS info_cont, info.information_name AS info_name, info.information_emailadd AS info_emailadd, info.information_id AS info_id, lgn.username AS uname, lgn.pword AS pword, info.role_id AS role, info.information_licno AS licno, info.information_address AS addr FROM tbl_information AS info INNER JOIN tbl_login AS lgn ON info.information_id = lgn.information_id WHERE info.information_id = ? AND info.information_is_active = ?");
